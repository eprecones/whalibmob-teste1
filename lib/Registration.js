...
function recordRegistrationResponse(store, response, source, method, now) {
  ...
  if (!Object.prototype.hasOwnProperty.call(state, 'consent')) state.consent = null;
  state.checkedAt = now;
...
function consentInteger(value) {
  if (value === null || value === undefined || value === '') return null;
  const number = Number(value);
  return Number.isInteger(number) && number >= 0 ? number : null;
}

function normalizeParentConsentUrl(value) {
  if (typeof value !== 'string' || !value.trim()) return null;
  try {
    const url = new URL(value.trim(), 'https://whatsapp.com/parent_consent/');
    const host = url.hostname.toLowerCase();
    if (url.protocol !== 'https:' || (host !== 'whatsapp.com' && host !== 'www.whatsapp.com')) {
      return null;
    }
    if (!url.pathname.startsWith('/parent_consent/')) return null;
    return url.toString();
  } catch (_) {
    return null;
  }
...
function consentGateDetails(result) {
  result = result || {};
  return {
    reason: typeof result.reason === 'string' ? result.reason : 'consent',
    pending: typeof result.pending === 'string' ? result.pending : 'app_store_age',
    consentId: consentInteger(result.consent_id),
    consentVersion: consentInteger(result.consent_version),
    parentConsentUrl: normalizeParentConsentUrl(result.parent_consent_url)
  };
}

function recordConsentGate(store, result) {
  recordRegistrationResponse(store, result, 'register', null);
  const state = currentRegistrationState(store);
  const details = consentGateDetails(result);
  if (state) {
    // Parent URLs can contain a one-time slug. Expose it only on the thrown
    // error; persist the non-secret identifiers needed to explain/resume the
    // official flow.
    state.consent = {
      reason: details.reason,
      pending: details.pending,
      consentId: details.consentId,
      consentVersion: details.consentVersion
    };
  }
  return details;
}
...
async function verifyCode(store, code, opts) {
  ...
  if (result.pending === 'app_store_age' || reason === 'consent') {
    ...
    store.codePending = false;
    store.codeMethod  = null;
    const consent = recordConsentGate(store, result);
    const canonical = result.login ? String(result.login).replace(/\D/g, '') : null;
    const lines = [
      'Verification stopped: WhatsApp requires the official app-store age/consent flow' +
        (consent.pending ? ' (pending: ' + consent.pending + ')' : '') + '.',
      '  The submitted code was accepted and consumed; do not reuse or resubmit it.',
      '  Complete the age or parent-consent step in the official Play Store/App Store installation.',
      '  This library does not fabricate date of birth, Play Age Signals, or a parent decision.'
    ];
    if (consent.parentConsentUrl) {
      lines.push('  The server supplied an official parent-consent URL in err.consent.parentConsentUrl.');
    }
    lines.push('  After the primary app finishes onboarding, link this library through companion pairing.');
...
module.exports._verify = {
  ...
  remainingRegistrationWait, consentGateDetails, recordConsentGate,
