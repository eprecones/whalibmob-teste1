...
function normalizeConsentState(value) {
  ...
  const pending = typeof value.pending === 'string' ? value.pending : null;
  const reason = typeof value.reason === 'string' ? value.reason : null;
  const consentId = value.consentId === null || value.consentId === undefined
    ? NaN
    : Number(value.consentId);
  const consentVersion = value.consentVersion === null || value.consentVersion === undefined
    ? NaN
    : Number(value.consentVersion);
  if (!pending && !reason && !Number.isInteger(consentId) && !Number.isInteger(consentVersion)) {
    return null;
  }
  return {
    pending,
    reason,
    consentId: Number.isInteger(consentId) && consentId >= 0 ? consentId : null,
    consentVersion: Number.isInteger(consentVersion) && consentVersion >= 0
      ? consentVersion
      : null
  };
}
...
function normalizeRegistrationState(value, accessSessionId) {
  ...
  const state = {
    ...
    fallbackMethods: Array.isArray(value.fallbackMethods)
      ...
      : [],
    consent: normalizeConsentState(value.consent)
  };
